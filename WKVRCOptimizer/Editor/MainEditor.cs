    ...
    public class AvatarOptimizerEditor : UnityEditor.Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (AvatarOptimizer.HasCustomShaderSupport)
    ...
    private bool Validate()
    {
        ...
        if ((optimizer.MergeSkinnedMeshesWithNaNimation || optimizer.MergeSkinnedMeshesWithShaderToggle)
            && optimizer.GetPolyCount() > AvatarOptimizer.MaxPolyCountForAutoShaderToggle)
        {
            EditorGUILayout.HelpBox(
                $"For avatars with a high poly count ({(int)(AvatarOptimizer.MaxPolyCountForAutoShaderToggle / 1000)}k or more)" +
        ...
        if (furyType != null && optimizer.GetComponentsInChildren(furyType, true).Any())
        {
            EditorGUILayout.HelpBox(
                ...
                $"For uploading use the {AvatarOptimizer.GetDisplayName(nameof(AvatarOptimizer.ApplyOnUpload))} feature as that ensures fury and the optimizer get used in the correct order.", MessageType.Warning);
            return false;
        }
        ...
        if (optimizer.GetComponentsInChildren<nadena.dev.modular_avatar.core.AvatarTagComponent>(true).Any())
        {
            EditorGUILayout.HelpBox(
                ...
                $"For uploading use the {AvatarOptimizer.GetDisplayName(nameof(AvatarOptimizer.ApplyOnUpload))} feature as that ensures Modular Avatar and the optimizer get used in the correct order.", MessageType.Warning);
            return false;
        }
        #endif
    ...
    {
        var property = typeof(AvatarOptimizer).GetProperty(propertyName);
        if (property == null)
            return false;
        var value = (bool)property.GetValue(optimizer);
        var output = value;
        var content = GetLabelWithTooltip(AvatarOptimizer.GetDisplayName(propertyName));
