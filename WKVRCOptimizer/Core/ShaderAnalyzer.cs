...
#pragma warning restore CS0649
        ...
        private bool RecursiveParseFile(string currentFileName, bool isTopLevelFile, string callerPath)
        {
            ...
            for (int lineIndex = 0; lineIndex < rawLines.Length; lineIndex++)
            {
                ...
                if (trimmedLine.StartsWithSimple("//"))
                {
                    ...
                    else if (trimmedLine.StartsWithSimple("WKVRCOptimizer:", 2))
                    {
                        ParseOptimizerComment(trimmedLine.Substring("//WKVRCOptimizer:".Length));
                    }
                    continue;
                }
                for (int i = 0; i < trimmedLine.Length; i++)
                {
                    ...
                    if (trimmedLine[i + 1] == '/')
                    {
                        if (trimmedLine.StartsWithSimple("WKVRCOptimizer:", i + 2))
                        {
                            ParseOptimizerComment(trimmedLine.Substring(i + 2 + "WKVRCOptimizer:".Length));
                        }
    ...
    public class ShaderOptimizer
    {
        public class OptimizedShader
        {
            ...
            {
                this.name = name;
                files[0].lines[0] = $"Shader \"whyknot/Optimizer/{name}\" // {originalShader.name}";
            }
        }
        ...
        private void InjectAnimatedPropertyInitialization()
        {
            foreach (var animatedProperty in animatedPropertyValues)
            {
                ...
                string animatedValue = localMeshCount > 1
                    ? $"{CBufferAliasArray[name].name}[{CBufferAliasArray[name].offset} + WKVRCOptimizer_MeshID]"
                    : $"WKVRCOptimizer{name}_ArrayIndex{mergedMeshIndices.First()}";
                if (animatedPropertyOnMeshID.TryGetValue(name, out var onMeshID))
                {
                    ...
                    {
                        output.Add($"{name} = WKVRCOptimizer{name}_IsAnimated[WKVRCOptimizer_MeshID] ? {animatedValue} : {name};");
                    }
                    else
                    {
                        output.Add($"{name} = ((1u << WKVRCOptimizer_MeshID) & WKVRCOptimizer{name}_IsAnimatedMask) != 0 ? {animatedValue} : {name};");
                    }
                }
                else
                {
                    output.Add($"{name} = isnan(asfloat(asuint({animatedValue}.x) ^ asuint(WKVRCOptimizer_Zero))) ? {name} : {animatedValue};");
                }
            }
        }
        ...
        private void AddParameterStructWrapper(List<string> funcParams, List<string> output, string name, bool addMeshMaterialID, bool isInput)
        {
            ...
            if (addMeshMaterialID)
                output.Add("uint WKVRCOptimizer_MeshMaterialID : WKVRCOptimizer_MeshMaterialID;");
            foreach (var line in funcParams)
            {
                ...
                if (match.Success)
                {
                    ...
                    if (match.Groups[4].Value.ToLowerInvariant().Contains("vface"))
                        output.Add("uint WKVRCOptimizer_SV_IsFrontFace : SV_IsFrontFace;");
                    else
        ...
        private void InitializeParameterFromWrapper(List<string> funcParams, List<string> output, string wrapperName, bool isInput)
        {
            foreach (var line in funcParams)
            {
                ...
                if (match.Success)
                {
                    ...
                    {
                        output.Add($"{type} {name} = {wrapperName}.WKVRCOptimizer_SV_IsFrontFace ? 1 : -1;");
                    }
        ...
        private void InjectVertexShaderCode(List<string> source, ref int sourceLineIndex)
        {
            ...
            if (needToPassOnMeshOrMaterialID)
            {
                ...
                output.Add("vertexOutputWrapper WKVRCOptimizer_vertexWithWrapper(vertexInputWrapper WKVRCOptimizer_vertexInput)");
                output.Add("{");
                InitializeParameterFromWrapper(funcParams, output, "WKVRCOptimizer_vertexInput", true);
                output.Add($"WKVRCOptimizer_MeshID = ((uint){inParam.name}.{vertexInUv0Member}.z >> 12) - {mergedMeshIndices.First()};");
                output.Add($"WKVRCOptimizer_MaterialID = 0xFFF & (uint){inParam.name}.{vertexInUv0Member}.z;");
                output.Add("vertexOutputWrapper WKVRCOptimizer_vertexOutput = (vertexOutputWrapper)0;");
                output.Add("WKVRCOptimizer_vertexOutput.WKVRCOptimizer_MeshMaterialID = WKVRCOptimizer_MaterialID | (WKVRCOptimizer_MeshID << 16);");
                nullReturn = "return WKVRCOptimizer_vertexOutput;";
            }
            else
            {
                int startLineIndex = sourceLineIndex;
            ...
            while (++sourceLineIndex < source.Count)
            {
                ...
                if (line == "}")
                {
                    if (--curlyBraceDepth == 0)
                    {
                        ...
                        {
                            output.Add("{");
                            InitializeParameterFromWrapper(funcParams, output, "WKVRCOptimizer_vertexOutput", false);
                            output.Add("return WKVRCOptimizer_vertexOutput;");
                            output.Add("}");
                        }
                        break;
                    }
            ...
            ref int sourceLineIndex)
        {
            ...
            if (usesInputWrapper)
            {
                ...
                wrapperStructs.Add("{");
                wrapperStructs.Add("uint WKVRCOptimizer_MeshMaterialID : WKVRCOptimizer_MeshMaterialID;");
                wrapperStructs.Add(inParam.type + " WKVRCOptimizer_geometryInput;");
                wrapperStructs.Add("};");
            }
            if (usesOutputWrapper)
            {
                ...
                wrapperStructs.Add("{");
                wrapperStructs.Add("uint WKVRCOptimizer_MeshMaterialID : WKVRCOptimizer_MeshMaterialID;");
                wrapperStructs.Add(outParamType + " WKVRCOptimizer_geometryOutput;");
                wrapperStructs.Add("};");
            }
            ...
            output.Add($"void {func.name}({geometryType} "
                + (usesInputWrapper ? inputWrapperName + " WKVRCOptimizer_inputWrapper" : inParam.type + " " + inParam.name)
                + $"[{inParam.arraySize}]"
            ...
            if (usesInputWrapper)
            {
                output.Add("WKVRCOptimizer_MaterialID = WKVRCOptimizer_inputWrapper[0].WKVRCOptimizer_MeshMaterialID & 0xFFFF;");
                output.Add("WKVRCOptimizer_MeshID = WKVRCOptimizer_inputWrapper[0].WKVRCOptimizer_MeshMaterialID >> 16;");
                output.Add(inParam.type + " " + inParam.name + "[" + inParam.arraySize + "];");
                for (int i = 0; i < inParam.arraySize; i++)
                {
                    output.Add(inParam.name + "[" + i + "] = WKVRCOptimizer_inputWrapper[" + i + "].WKVRCOptimizer_geometryInput;");
                }
            }
            ...
            while (++sourceLineIndex < source.Count)
            {
                ...
                {
                    output.Add("{");
                    output.Add(outputWrapperName + " WKVRCOptimizer_geomOutput;");
                    output.Add("WKVRCOptimizer_geomOutput.WKVRCOptimizer_geometryOutput = " + line.Substring(line.IndexOf(".Append(") + 7));
                    output.Add("WKVRCOptimizer_geomOutput.WKVRCOptimizer_MeshMaterialID = "
                        + "WKVRCOptimizer_MaterialID | (WKVRCOptimizer_MeshID << 16);");
                    output.Add(outParam.name + ".Append(WKVRCOptimizer_geomOutput);");
                    output.Add("}");
                }
                else
                {
            ...
            ref int sourceLineIndex)
        {
            ...
            if (arrayPropertyValues.Count > 0 || animatedPropertyValues.Count > 0)
            {
                ...
                output.Add("fragmentInputWrapper WKVRCOptimizer_fragmentInput");
                ...
                if (func.parameters[0].semantic != null)
                    ...
                    output.Add(")");
                output.Add("{");
                InitializeParameterFromWrapper(funcParams, output, "WKVRCOptimizer_fragmentInput", true);
        ...
        private void InjectDummyTextureUsage()
        {
            ...
            output.Add("if (WKVRCOptimizer_Zero)");
            output.Add("{");
            output.Add("float WKVRCOptimizer_sum = 0;");
            foreach (var tex in texturesToCallSoTheSamplerDoesntDisappear)
            {
                ...
                {
                    output.Add($"WKVRCOptimizer_sum += {tex}.Sample(sampler{tex}, 0).x;");
                }
                else
                {
                    output.Add($"WKVRCOptimizer_sum += {tex}.Load(0).x;");
                }
                output.Add("#endif");
            }
            output.Add($"if (WKVRCOptimizer_sum) {dummyTextureNullReturn}");
            output.Add("}");
        ...
        private void InjectDummyCBufferUsage(string nullReturn)
        {
            ...
            valuesToDummyUse.Push("WKVRCOptimizerAnimatedScalars[WKVRCOptimizer_MeshID]");
            if (hasVectorCBufferAliasArray)
                valuesToDummyUse.Push("WKVRCOptimizerAnimatedVectors[WKVRCOptimizer_MeshID].x");
            ...
            {
                string propName = $"WKVRCOptimizer{animatedProperty}_ArrayIndex";
                for (int i = 0; i < mergedMeshIndices.Count; i++)
                {
            ...
            output.Add("if (WKVRCOptimizer_Zero)");
            output.Add("{");
            output.Add($"float WKVRCOptimizer_val = {valuesToDummyUse.Pop()};");
            while (valuesToDummyUse.Count >= 2)
            {
                output.Add($"WKVRCOptimizer_val += {valuesToDummyUse.Pop()} * {valuesToDummyUse.Pop()};");
            }
            if (valuesToDummyUse.Count == 1)
            {
                output.Add($"WKVRCOptimizer_val += {valuesToDummyUse.Pop()};");
            }
            output.Add("if (WKVRCOptimizer_val) " + nullReturn);
            output.Add("}");
        }
        ...
        private void InjectIsActiveMeshCheck(string nullReturn)
        {
            if (mergedMeshCount <= 1)
                return;
            if (localMeshCount > 1)
                output.Add($"if (0.5 > WKVRCOptimizerAnimatedScalars[WKVRCOptimizer_MeshID]) {nullReturn}");
            else
        ...
        private void InjectPropertyArrays()
        {
            ...
            if (mergedMeshCount > 1)
            {
                ...
                CBufferAliasArray.Add("_IsActiveMesh", ("WKVRCOptimizerAnimatedScalars", 0));
                ...
                foreach (var animatedProperty in animatedPropertyValues)
                {
                    ...
                    if (type[type.Length - 1] == '4')
                    {
                        ...
                        CBufferAliasArray.Add(name, ("WKVRCOptimizerAnimatedVectors", currentPackOffset));
                    }
                    else
                    {
                        type = "float";
                        currentPackOffset = currentScalarPackOffset = AllocateCBufferRegisters(currentScalarPackOffset, usedScalarRegisters);
                        CBufferAliasArray.Add(name, ("WKVRCOptimizerAnimatedScalars", currentPackOffset));
                    }
                    foreach (int i in mergedMeshIndices)
                    {
                        currentOutput.Add($"{type} WKVRCOptimizer{name}_ArrayIndex{i} : packoffset(c{currentPackOffset + i - mergedMeshIndices.First()});");
                    }
                }
                output.Add("cbuffer WKVRCOptimizerAnimatedScalars");
                output.Add("{");
                if (localMeshCount > 1)
                    output.Add($"float WKVRCOptimizerAnimatedScalars[{usedScalarRegisters.Max() + 1}] : packoffset(c0);");
                output.AddRange(scalarOutput);
                output.Add("};");
                if (hasVectorCBufferAliasArray)
                {
                    output.Add("cbuffer WKVRCOptimizerAnimatedVectors");
                    output.Add("{");
                    if (localMeshCount > 1)
                        output.Add($"float4 WKVRCOptimizerAnimatedVectors[{usedVectorRegisters.Max() + 1}] : packoffset(c0);");
                ...
                foreach (var animatedPropertyMask in animatedPropertyOnMeshID)
                {
                    ...
                    {
                        output.Add($"static const bool WKVRCOptimizer{name}_IsAnimated[{localMeshCount}] = ");
                        output.Add("{");
                    ...
                    else
                    {
                        ...
                        output.Add($"static const uint WKVRCOptimizer{name}_IsAnimatedMask = {bitField};");
                    }
                }
            }
                ...
                switch (identifier) {
                    case "vertex":
                        if ((currentPass.geometry != null && mergedMeshCount > 1) || arrayPropertyValues.Count > 0 || animatedPropertyValues.Count > 0) {
                            pragmaOutput.Add("#pragma vertex WKVRCOptimizer_vertexWithWrapper");
                        } else {
            ...
            while (lineIndex < lines.Count)
            {
                ...
                else if (line == "{")
                {
                    ...
                    foreach (var animatedProperty in animatedPropertyValues)
                    {
                        ...
                        foreach (int i in mergedMeshIndices)
                        {
                            var fullPropertyName = $"WKVRCOptimizer{prop.name}_ArrayIndex{i}";
                    ...
                    foreach (var defaultAnimatedProperty in defaultAnimatedProperties)
                    {
                        ...
                        {
                            var match = Regex.Match(defaultAnimatedProperty.name, @"WKVRCOptimizer(\w+)_ArrayIndex(\d+)");
                            if (match.Success)
                            {
